...
def _admin_query(admin_id: str) -> dict:
    cleaned = str(admin_id or "").strip()
    clauses = [{"id": cleaned}]
...
async def get_admin_by_id(admin_id: str):
    if db is None:
        return None
    return await db.admins.find_one(_admin_query(admin_id))
...
def admin_doc_to_public(doc: dict) -> AdminBase:
    admin_id = str(doc.get("id") or doc.get("_id") or "")
    return AdminBase(
        id=admin_id,
