      ...
      <section className="pt-32 pb-24">
        <div className="max-w-md mx-auto px-6">
          <div className="rounded-[1.75rem] border border-violet-500/15 bg-[#0d0820]/80 p-8 shadow-[0_30px_80px_rgba(120,80,255,0.12)]">
            ...
            <h1 className="text-3xl font-bold tracking-tight">Join PranvithDOP</h1>
            <p className="mt-2 text-sm text-white/60">Sign up to unlock downloads, track orders, and manage purchases.</p>
            ...
            <form onSubmit={onSubmit} className="mt-7 space-y-4" data-testid="register-form">
              ...
              {err && (
                <div data-testid="register-error" className="rounded-lg border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-sm text-rose-200">
                  {err}
                </div>
              )}
              ...
              >
                {busy ? <><Loader2 size={14} className="animate-spin" /> Creating account...</> : 'Create account'}
              </button>
            </form>

            <p className="mt-6 text-center text-sm text-white/60">
              Already have an account?{' '}
              <Link to="/login" className="text-violet-300 hover:text-white font-semibold" data-testid="register-go-login">
                Sign in
              </Link>
            </p>
          </div>
