...
const About = () => {
  ...
  const stats = legacySections?.stats || defaultStats;
    ...
    <section className="pt-12 pb-24 px-6">
      <div className="max-w-5xl mx-auto text-center">
        ...
        <p className="mt-7 text-white/70 text-lg leading-relaxed max-w-3xl mx-auto">
          {intro.headline ? `${intro.headline} ${intro.description || ''}` :
            `PranvithDOP is the home for aspiring video editors, content creators, and freelancers who want to learn professional editing techniques from real-world projects. We combine industry-standard tools with AI-powered workflows to help you create stunning videos faster than ever.`}
        </p>
      </div>
      ...
      <div className="max-w-3xl mx-auto mt-20 text-white/75 leading-relaxed space-y-5 text-base">
        {(legacySections?.content || [
          'Founded by Pranavith from Hyderabad, India, PranvithDOP started as a YouTube channel sharing free video editing tutorials. Today it has grown into a full-fledged education platform with structured courses, real-world projects, mentorship, and a thriving community of creators.',
          'Our mission is simple: make professional video editing accessible to everyone, whether you are a beginner taking your first steps, a YouTuber leveling up your content, or a freelancer building a career out of your craft.',
        ]).map((text, idx) => (
          <p key={idx}>{text}</p>
        ))}
      </div>
