...
const About = () => {
  ...
  return (
    <main className="page bg-[#070314] text-white">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          ...
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              ...
              <Link to={safePublicHref(hero.data?.secondary_button_link, '/works')} className="rounded-full border border-white/15 px-7 py-3 text-sm font-semibold text-white hover:bg-white/10">{hero.data?.secondary_button_text || 'View portfolio'}</Link>
            </div>
          </div>
        </div>
      </section>}
      ...
      {(creativeSection.section_id || gearSection.section_id) && <section className="px-6 pb-24">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-2">
          {creativeSection.section_id && <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-8">
            <Sparkles className="mb-5 text-violet-300" />
            <h2 className="text-3xl font-bold text-white">{creativeSection.title || 'Creative positioning'}</h2>
            <p className="mt-4 leading-relaxed text-white/70">
              {creativeSection.description}
            </p>
          </div>}
          {gearSection.section_id && <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-8">
            <Award className="mb-5 text-violet-300" />
            <h2 className="text-3xl font-bold text-white">{gearSection.title || 'Gear & workflow'}</h2>
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {gearItems.map((item) => (
                <div key={item.title} className="rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white/75">{item.title}</div>
              ))}
            </div>
          </div>}
        </div>
