...
const SectionOrderList = ({ sections, selectedId, onSelect, onMove, onDelete, onVisibilityChange, disabled = false }) => (
    ...
    {sections.map((section, index) => (
      <div key={section.id} className={`rounded-2xl border p-4 ${selectedId === section.id ? 'border-violet-500 bg-violet-500/10' : 'border-slate-800 bg-slate-900'}`}>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <button type="button" disabled={disabled || index === 0} onClick={() => onMove(index, -1)} className="rounded-lg border border-slate-700 p-2 text-slate-200 disabled:opacity-30"><ArrowUp size={14} /></button>
          <button type="button" disabled={disabled || index === sections.length - 1} onClick={() => onMove(index, 1)} className="rounded-lg border border-slate-700 p-2 text-slate-200 disabled:opacity-30"><ArrowDown size={14} /></button>
